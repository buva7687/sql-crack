...
const SESSION_COMMAND_PATTERNS: Array<{
    ...
    { pattern: /^ALTER\s+TABLE\s+(\S+)\s+APPEND\s+FROM\s+(\S+)/i, type: 'ALTER TABLE APPEND', description: (m) => `Append from ${m[2]} into ${m[1]}`, dialects: ['Redshift'] },
    ...
    { pattern: /^VACUUM\s*(.*)/i, type: 'VACUUM', description: (m) => `Vacuum${m[1] ? ': ' + m[1] : ''}`, dialects: ['PostgreSQL', 'Redshift'] },
    { pattern: /^ANALYZE\s*(.*)/i, type: 'ANALYZE', description: (m) => `Analyze${m[1] ? ': ' + m[1] : ''}`, dialects: ['PostgreSQL', 'Redshift'] },
