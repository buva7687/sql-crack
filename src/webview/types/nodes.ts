...
export type NodeType = 'table' | 'filter' | 'join' | 'aggregate' | 'sort' | 'limit' | 'select' | 'result' | 'cte' | 'union' | 'subquery' | 'window' | 'case' | 'cluster' | 'operation';
...
export interface FlowNode {
    ...
    tableCategory?: 'physical' | 'derived' | 'cte_reference' | 'table_function' | 'subquery_source';
...
export interface FlowEdge {
    ...
    label?: string;
    sqlClause?: string;
    clauseType?: 'join' | 'where' | 'having' | 'on' | 'filter' | 'flow' | 'merge_source' | 'merge_target' | 'subquery_flow';
    startLine?: number;
    endLine?: number;
}
